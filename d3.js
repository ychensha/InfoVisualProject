d3 = function() {
  ...
  d3_hslPrototype.brighter = function(k) {
    ...
    return d3_hsl(this.height, this.s, this.l / k);
  };
  d3_hslPrototype.darker = function(k) {
    k = Math.pow(.7, arguments.length ? k : 1);
    return d3_hsl(this.height, this.s, k * this.l);
  };
  d3_hslPrototype.rgb = function() {
    return d3_hsl_rgb(this.height, this.s, this.l);
  };
  ...
  d3_hclPrototype.brighter = function(k) {
    return d3_hcl(this.height, this.c, Math.min(100, this.l + d3_lab_K * (arguments.length ? k : 1)));
  };
  d3_hclPrototype.darker = function(k) {
    return d3_hcl(this.height, this.c, Math.max(0, this.l - d3_lab_K * (arguments.length ? k : 1)));
  };
  d3_hclPrototype.rgb = function() {
    return d3_hcl_lab(this.height, this.c, this.l).rgb();
  };
  ...
  function d3_geo_centroidLineStart() {
    ...
    function nextPoint(λ, φ) {
      λ *= d3_radians;
      var cosφ = Math.cos(φ *= d3_radians), x = cosφ * Math.cos(λ), y = cosφ * Math.sin(λ), z = Math.sin(φ), w = Math.atan2(Math.sqrt((width = y0 * z - z0 * y) * width + (width = z0 * x - x0 * z) * width + (width = x0 * y - y0 * x) * width), x0 * x + y0 * y + z0 * z);
      d3_geo_centroidW += w;
  ...
  d3.svg.brush = function() {
    ...
    function redrawY(g) {
      ...
      g.selectAll(".extent,.e>rect,.width>rect").attr("height", extent[1][1] - extent[0][1]);
    }
    function brushstart() {
  ...
  var d3_svg_brushResizes = [ [ "n", "e", "s", "width", "nw", "ne", "se", "sw" ], [ "e", "width" ], [ "n", "s" ], [] ];
  d3.time = {};
